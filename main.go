...
var (
	...
	flagSSLEmail = flag.String("email", "you@emailprovider.com", "email for letsencrypt expiry notifications")
)
...
func main() {
	...
	// setup transform routes
	initTrx()

	// check if TLS was enabled, otherwise serve HTTP
	if !*flagTLS {
		s := makeServer()
		log.Fatal(s.ListenAndServe())
	}

	// configure simplecert
	cfg.Domains = []string{*flagDomain}
	cfg.CacheDir = "letsencrypt"
	cfg.SSLEmail = *flagSSLEmail
